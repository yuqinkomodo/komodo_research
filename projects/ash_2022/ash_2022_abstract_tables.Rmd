...
ft <- flextable(table1, cwidth = 1) %>%
  ...
  add_footer_lines(values = 'CCI - Charlson Comorbidity Index; VOC - Vaso-Occlusive Crisis; PDC - Proportion of Days Covered') %>%
...
ft <- flextable(table2, cwidth = 1) %>%
  ...
  width(j = 3, width = 1) %>%
  # autofit(add_w = 0.0) %>%
  add_footer_lines(values = 'CCI - Charlson Comorbidity Index; VOC - Vaso-Occlusive Crisis') %>%
  bold(part = "header", i = 1) %>%
  align(part = "header", i = 1, align = "left") %>%
  border(part = "head", i = 1, border = list("width" = 0, color = "black", style = "solid")) %>%
  font(fontname = 'Times New Romans', part = 'all') %>%
...
# save_as_image(ft, file.path(plot_dir, 'table2.png'), webshot = "webshot2")
ft
```