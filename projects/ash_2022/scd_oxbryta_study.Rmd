...
```{r code_list00}
print(adakveo_proc <- read_query(get_code("crizanlizumab", method = "name", type = 'proc')))
sheet_write(adakveo_proc, ss = code_list_ss, sheet = 'adakveo_proc')
```
...
code_list <- 
  bind_rows(
    ...
    adakveo_proc %>% select(code, code_description) %>% mutate(codetype = "HCPCS", event = "adakveo"),
...
sql = glue("
  ...
  select upk_key2, start_date, end_date, 
    greatest(start_date + 365, to_date('2017-01-01')) as first_index_date,
...
```{r step_11_check}
mean_voc_df <- cohort %>% 
  group_by(cohort, year_quarter) %>% 
  summarize(
    n = n(),
    mean_ip_pre = mean(n_pre_voc_ip, na.rm = T), 
    mean_ip_post = mean(n_post_voc_ip, na.rm = T), 
    diff_ip = mean(n_post_voc_ip-n_pre_voc_ip, na.rm = T),
    mean_er_pre = mean(n_pre_voc_er, na.rm = T), 
    mean_er_post = mean(n_post_voc_er, na.rm = T),
    diff_er = mean(n_post_voc_er-n_pre_voc_er, na.rm = T)
    ) %>% 
  print.data.frame()
sheet_write(mean_voc_df, ss = output_ss, sheet = 'average_voc')
mean_voc_plot_df <-
  mean_voc_df %>% 
  select(cohort, year_quarter, mean_ip_pre, mean_ip_post, mean_er_pre, mean_er_post) %>%
  pivot_longer(cols = c("mean_ip_pre", "mean_ip_post", "mean_er_pre", "mean_er_post"), 
               names_to = "variable", values_to = "mean") %>%
  mutate(panel = ifelse(variable %in% c('mean_ip_post', 'mean_er_post'), 'post', 'pre'),
         outcome = ifelse(variable %in% c('mean_ip_post', 'mean_ip_pre'), 'ip', 'er')) %>%
  arrange(outcome, panel, cohort, year_quarter)

mean_voc_plot <- ggplot(mean_voc_plot_df) + 
  geom_line(aes(x = year_quarter, y = mean, color = panel, group = panel)) +
  facet_grid(cohort ~ outcome, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90))
print(mean_voc_plot)
ggsave(mean_voc_plot, filename = file.path(plot_dir, 'average_voc_by_quarter.png'), width = 6, height = 5)
drive_plot4 <- drive_upload(file.path(plot_dir, 'average_voc_by_quarter.png'), path = plot_drive, overwrite = T)
```

```{r step_11_check2}
mean_voc_df_trans <- cohort %>% 
  filter(n_transfusion >= 1) %>%
  group_by(cohort, year_quarter) %>% 
  summarize(
    n = n(),
    mean_ip_pre = mean(n_pre_voc_ip, na.rm = T), 
  ...
  print.data.frame()
sheet_write(mean_voc_df_trans, ss = output_ss, sheet = 'average_voc_trans')

mean_voc_plot_df_trans <-
  mean_voc_df_trans %>% 
  select(cohort, year_quarter, mean_ip_pre, mean_ip_post, mean_er_pre, mean_er_post) %>%
  pivot_longer(cols = c("mean_ip_pre", "mean_ip_post", "mean_er_pre", "mean_er_post"), 
               names_to = "variable", values_to = "mean") %>%
...
mean_voc_plot_trans <- ggplot(mean_voc_plot_df_trans) + 
  geom_line(aes(x = year_quarter, y = mean, color = panel, group = panel)) +
  facet_grid(cohort ~ outcome, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90))

print(mean_voc_plot_trans)
ggsave(mean_voc_plot_trans, filename = file.path(plot_dir, 'average_voc_trans_by_quarter.png'), width = 6, height = 5)
drive_plot5 <- drive_upload(file.path(plot_dir, 'average_voc_trans_by_quarter.png'), path = plot_drive, overwrite = T)
```
...
pre_matching_2 %>% group_by(cohort, ) %>%
  summarize(n = n(), mean(n_pre_voc_ip), mean(n_post_voc_ip))

post_matching_2 %>% group_by(year_quarter, cohort) %>%
  summarize(n = n(), mean(n_pre_voc_ip), mean(n_post_voc_ip))

post_matching_2 %>% group_by(year_quarter, cohort) %>%
  summarize(n = n(), mean(n_pre_voc_er), mean(n_post_voc_er))
```
